...
def test_build_subproject(master_index, app):
    ...
    assert_node(index_toctree[0][0],
                [compact_paragraph, reference, "Lorem Ipsum"])
    assert_node(index_toctree[0][0][0], reference, anchorname='')
    assert_node(index_toctree[0][1][0], toctree, caption=None, 
                                                 glob=False,
                                                 hidden=False,
                                                 includehidden=False,
                                                 titlesonly=False,
                                                 maxdepth=-1,
                                                 entries=[(None, 'http://example/en/latest/amet.html'), 
...
@pytest.mark.sphinx('html', testroot='subprojecttoctree-subproject')
def test_build_subproject_multiple_master_toctrees(app_params, make_app, mocker, caplog):
    master_index = dedent("""\
                          Dolor sit
                          =========
    ...
    with pytest.raises(SystemExit) as err:
        app.build()
    assert caplog.record_tuples == [('subprojecttoctree',
                                     logging.ERROR,
...
def test_build_subproject_url_not_set(master_index, app_params, make_app, caplog):
    ...
    with pytest.raises(SystemExit) as err:
        app.build()
    assert caplog.record_tuples == [('subprojecttoctree.utils',
                                     logging.ERROR,
                                     "The 'readthedocs_url' config value must be set.")]

@pytest.mark.sphinx('html', testroot='subprojecttoctree-subproject')
def test_build_subproject_is_subproject_not_set(master_index, app_params, make_app, caplog):
    args, kwargs = app_params
    app = make_app(*args, **kwargs)
...
@pytest.mark.sphinx('html', testroot='subprojecttoctree-subproject')
def test_build_subproject_url_wrong_protocol(master_index, app_params, make_app, caplog):
    args, kwargs = app_params
    app = make_app(*args, **kwargs)
